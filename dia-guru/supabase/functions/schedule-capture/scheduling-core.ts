...
import {
  applyRoutinePriorityScore,
  type CapturePrioritySnapshot,
  detectRoutineKind,
  priorityForCapture,
  type RoutineKind,
} from "./priority-model.ts";
export {
  detectRoutineKind,
  priorityForCapture,
  type RoutineKind,
...
export function resolveSleepBaseReference(
  ...
  const fromStartTarget = capture.start_target_at
    ? parseIsoDate(capture.start_target_at)
    : null;
  if (fromStartTarget) return fromStartTarget;

  const fromOriginal = capture.original_target_time
    ? parseIsoDate(capture.original_target_time)
    : null;
...
export function normalizeRoutineCapture(
  input: CaptureEntryRow,
  options: { referenceNow: Date; timezone?: string },
) {
  const capture = { ...input };
  const { referenceNow } = options;

  const isSleep = capture.task_type_hint === "routine.sleep" ||
    capture.extraction_kind === "routine.sleep";
  const isMeal = capture.task_type_hint === "routine.meal" ||
    capture.extraction_kind === "routine.meal";
  const isRoutine = isSleep || isMeal || isRoutineKind(capture.task_type_hint);

  if (!isRoutine) return capture;

  const userLocked = Boolean(capture.manual_touch_at) ||
    Boolean(capture.freeze_until);
...
export function findNextAvailableSlot(
    ...
    for (let day = 0; day < SEARCH_DAYS; day++) {
      ...
      for (const pref of options.preferredTimeOfDay) {
        ...
        const effectiveStart = new Date(
          Math.max(windowStart.getTime(), cursor.getTime()),
        );
  ...
  for (let day = 0; day < SEARCH_DAYS; day++) {
    ...
    let candidateStart = new Date(
      Math.max(dayStart.getTime(), cursor.getTime()),
    );
...
export function buildOccupancyGrid(args: {
  ...
  const totalDays = Math.max(
    1,
    Math.min(SEARCH_DAYS, args.days ?? SEARCH_DAYS),
  );
  ...
  for (let day = 0; day < totalDays; day++) {
    ...
    for (
      ...
      cursor.getTime() < dayEndUtc.getTime();
...
export function collectGridWindowCandidates(args: {
  ...
  const slotsNeeded = Math.max(
    1,
    Math.ceil(args.durationMinutes / slotMinutes),
  );
  ...
  for (let i = 0; i < grid.slots.length; i++) {
    ...
    const candidate = analyzeGridWindow(
      grid.slots,
      i,
      slotsNeeded,
      slotMinutes,
...
export function generateChunkDurations(args: {
  ...
  const totalMinutes = Math.max(
    increment,
    roundUpToIncrement(args.totalMinutes, increment),
  );
  ...
  const minChunkMinutes = Math.max(
    increment,
    roundUpToIncrement(
      args.minChunkMinutes ?? DEFAULT_MIN_CHUNK_MINUTES,
      increment,
    ),
  );
  const totalIncrements = Math.max(1, Math.ceil(totalMinutes / increment));
  const minChunkIncrements = Math.max(
    1,
    Math.ceil(minChunkMinutes / increment),
  );
  ...
  const maxChunksFromDuration = Math.floor(
    totalIncrements / minChunkIncrements,
  );
  ...
  while (
    chunkCount > 1 &&
    Math.floor(totalIncrements / chunkCount) < minChunkIncrements
  ) {
    chunkCount -= 1;
  }
...
function findSlotWithinRange(
  ...
  while (candidateStart.getTime() + durationMs <= limit) {
    ...
    if (enforceWorkingWindow && isAfterDayEnd(candidateStart, offsetMinutes)) {
      const nextDay = startOfDayOffset(
...
export function findLatePlacementSlot(args: {
  ...
  const startFrom = new Date(
    Math.max(args.startFrom.getTime(), args.referenceNow.getTime()),
  );
...
export function buildDeadlineFailurePayload(args: {
  ...
  return {
    error: args.reason === "slot_exceeds_deadline"
...
export function computeDateDeadline(
...
export function computeSchedulingPlan(
  ...
  if (
    constraintType === "deadline_time" || constraintType === "deadline_date"
  ) {
  ...
  if (constraintType === "start_time") {
    const targetStart = parseIsoDate(capture.constraint_time) ??
      parseIsoDate(capture.original_target_time);
    if (targetStart) {
      const start = new Date(
        Math.max(targetStart.getTime(), referenceNow.getTime()),
      );
...
function adjustSlotToReference(
  slot: PreferredSlot,
  referenceNow: Date,
): PreferredSlot {
...
export function isSlotFeasible(
  ...
  if (enforceWorkingWindow && isBeforeDayStart(slot.start, offsetMinutes)) {
    return false;
  }
  if (enforceWorkingWindow && isAfterDayEnd(slot.end, offsetMinutes)) {
    return false;
  }
...
export function findSlotBeforeDeadline(
  ...
  if (enforceWorkingWindow && isAfterDayEnd(candidateStart, offsetMinutes)) {
    candidateStart = startOfDayOffset(
      addDays(candidateStart, 1),
      offsetMinutes,
    );
  }
...
export function findSlotWithinWindow(
  ...
  const durationMs = durationMinutes * 60000;
  let candidateStart = new Date(
    Math.max(options.windowStart.getTime(), options.referenceNow.getTime()),
  );
...
export function isSlotWithinConstraints(
  capture: CaptureEntryRow,
  ...
  if (capture.constraint_type === "deadline_time") {
    pushIfValid(capture.constraint_time);
  }
  if (candidates.length === 0) return true;
  const minEnd = new Date(Math.min(...candidates.map((d) => d.getTime())));

  if (
    capture.constraint_type === "start_time" && capture.window_start &&
    capture.window_end
  ) {
    const window_start = new Date(capture.window_start);
    const window_end = new Date(capture.window_end);
    if (
      !Number.isNaN(window_start.getTime()) &&
      !Number.isNaN(window_end.getTime())
    ) {
...
export function withinStabilityWindow(
  capture: CaptureEntryRow,
  referenceNow: Date,
) {
  ...
  return plannedTs - referenceNow.getTime() <=
    STABILITY_WINDOW_MINUTES * 60_000;
}
  ...
  for (const buffer of uniqueBuffers) {
    for (let size = 1; size <= maxCombinationSize; size++) {
      ...
      for (const combo of combos) {
        ...
        const filteredEvents = args.events.filter((event) =>
          !removalSet.has(event.id)
        );
...
export function estimateConflictMinutes(
...
export function applyRoutinePrioritySnapshot(
  snapshot: CapturePrioritySnapshot,
  kind: RoutineKind,
  durationMinutes: number,
) {
...
export function readCannotOverlapFromNotes(capture: CaptureEntryRow): boolean {
  if (typeof capture.cannot_overlap === "boolean") {
    return Boolean(capture.cannot_overlap);
  }
  try {
    const raw = typeof capture.scheduling_notes === "string"
...
export function isSlotWithinWorkingWindow(
  slot: PreferredSlot,
  offsetMinutes: number,
) {
